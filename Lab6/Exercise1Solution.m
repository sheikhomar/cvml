...
for i=1:numframes-1 % number of images to go through
    ...
    for j=1:numPoints       %number of points to consider
        ...
        while stopscore > .1 && count < 40;  %iterate until convergence on a point
            ...
            % Exercise: construct the matrices 'M' and 'b' to find the displacement (u,v). 
            % Hint: use 'Ix', 'Iy' and 'It' defined above.
            % See slide 37-41 in Lecture5_OpticalFlow.pdf
        ...
        vectorized = 0;
        if vectorized
        % Estimate affine transformation: Vectorized implementation
            if i==1 % Only estimate 'A' in frame 1. It does not depend on the following frames.
                % Alternative, vectorized implementation
                xr = -wsize:wsize;
                yr = -wsize:wsize;
                [xrangeA, yrangeA] = meshgrid(xr,yr);
                A = [Ix(:), Ix(:).*yrangeA(:), Ix(:).*xrangeA(:), Iy(:), Iy(:).*yrangeA(:), Iy(:).*xrangeA(:)];
                LSSconst{j} = inv(A'*A)*A';
            end
            
            % Estimate 'b' for each frame.
            b = -It(:);
            
            % Least squares solution: a = inv(A'*A)*A'*b
            a = LSSconst{j}*b;
        else
            ...
            for y=-wsize:wsize
                for x=-wsize:wsize
                    ...
                    A((2*wsize+1)*(wsize+y)+wsize+x+1,:) = [IX IX*x IX*y IY IY*x IY*y];
                    
                    % We need to compute b for all frames. Therefore, we
                    % don't do it here. However, it could be done in a
                    % similar loop like this:
                    b((2*wsize+1)*(wsize+y)+wsize+x+1) = -It(wsize+x+1,wsize+y+1);
                end
            end
